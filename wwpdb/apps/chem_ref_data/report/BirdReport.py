...
class BirdReport(object):
    ...
    def doReport(self):
        ...
        logger.info("+BirdReport.doReport()  - file path    %s\n" % filePath)
        logger.info("+BirdReport.doReport()  - file format  %s\n" % fileFormat)
        logger.info("+BirdReport.doReport()  - block Id     %s\n" % blockId)
        #
        #
        ...
        if (filePath != localPath):
            ...
            #
            logger.info("+BirdReport.doReport() - Copied input file %s to report session path %s \n" % (filePath, localPath))
        ...
        try:
            ...
            if self.__prdId.startswith('PRD_'):
                ...
                #
                if (self.__verbose):
                    logger.info("+BirdReport.doReport() - PRD %r CC ID %r path to image %r\n" % (self.__prdId, tId, pathToImage))

            oD['blockId'] = prdR.getCurrentContainerId()
            if (self.__verbose):
                logger.info("+BirdReport.doReport() - category name list %r \n" % prdR.getCurrentCategoryNameList())
            ...
            logger.exception("Report preparation failed for:  %s\n" % fileName)

        #
        try:
            prdCcFilePath = pathToImage
            if prdCcFilePath is not None:
                (pth, prdCcFileName) = os.path.split(prdCcFilePath)
                ...
                #
                logger.info("+BirdReport.doReport() - Copied input PRDCC file %s to report session path %s \n" % (prdCcFilePath, localPrdCcPath))
            ...
            logger.info("Category name list %r \n" % ccR.getCurrentCategoryNameList())
