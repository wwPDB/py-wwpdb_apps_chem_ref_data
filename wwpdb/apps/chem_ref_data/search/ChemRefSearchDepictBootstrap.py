...
class ChemRefSearchDepictBootstrap(ChemRefDataDepictBootstrap):
    ...
    def doAltRenderCollapsable(self, d={}, searchName="Search results", compareType=None, searchTarget=None):
    ...
    def __panelBegin(self, panelTitle, state="collapsed", toggleStyle="href"):
        ...
        if True:
            oL.append(
                '         <button type="button" class="close" data-target="#%s" data-dismiss="alert"> <span aria-hidden="true"> <i class="fa fa-times"></i></span><span class="sr-only">Close</span></button>' %
                idPanel)
            ...
            for cName in rD['columnList']:
                if cName in result and result[cName]:
                    if cName in ['Family ID', 'PRD ID']:
                        ...
                        le = '<a target="_blank" href="http://ligand-expo.rcsb.org/pyapps/ldHandler.py?formid=cc-index-search&operation=ccid&target=%s">(LE)</a>' % result[
                            cName]
    ...
    def __bootstrapTableTemplate(self, colNameTupList, columnWidthList, title="Search Results",
                                 tableId="chemref-bst-result-table-1", divId='chemref-bst-result-div-1', pFunc="pagerFunc"):
        """
        ...
        tableH = '''<div id="%s" class="row">
           ...
           data-show-refresh="true"
           data-striped="true"
                     data-show-toggle="true">''' % (divId, tableId, pFunc)
        # omit
        #            data-height="600"
        #
        oL = []
    ...
    def doBsTableRenderCollapsable(self, d={}, searchName="Search results", compareType=None, searchTarget=None):
    ...
    def doBsTableRenderResults(self, d={}, title="Search Results Summary"):
        '''
        ...
        for resultSetId in range(1, len(d['resultDictionary']) + 1):
            ...
            #
            oL.extend(
                self.__bootstrapTableTemplate(
                    colNameTupList,
                    rD['columnWidthList'],
                    title=t,
                    tableId=tD['resultSetTableId'],
                    divId=tD['resultSetContainerId'],
                    pFunc="pagerFunc"))
            #
    ...
    def doAltRenderResults(self, d={}, title="Search Results Summary"):
        ''' Render in HTML
        '''
        #
        sp = '&nbsp;&nbsp;'
            ...
            for result in rD['resultlist']:
                ...
                for cName in rD['columnList']:
                    if cName in result and result[cName]:
                        if (cName == 'Family ID'):
                            oL.append(
                                '          <td><a class="app-ref-report"  href="#">%s</a></td>' %
                                (result[cName]))
                        elif (cName == 'PRD ID'):
                            oL.append(
                                '          <td><a class="app-ref-report"   href="#">%s</a></td>' %
    ...
    def doRenderCollapsable(self, d={}, title="Search results", compareType=None, searchTarget=None):
        ''' Render in collapsable section -
        '''
        oL = []
        if d['count'] < 1:
    ...
    def doRenderResults(self, d={}, title="Search Results Summary"):
        ''' Render in HTML
        '''
        #
        sp = '&nbsp;&nbsp;'
