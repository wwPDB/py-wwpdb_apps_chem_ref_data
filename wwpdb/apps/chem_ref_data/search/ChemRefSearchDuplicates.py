...
class ChemRefSearchDuplicates(MyConnectionBase):
    ...
    def find_duplicates(self):
        ...
        self.__dups = []
        for row in rows:
            d = {'num': row[0], 'cid1': row[1], 'cid1status': row[2], 'cid1site': row[3], 'cid1mod': row[4],
                 'cid2': row[5], 'cid2status': row[6], 'cid2site': row[7], 'cid2mod': row[8]}
    ...
    def _rformat(self, row):
        ...
        return ("{} {:<3} {:<4} {:<4} {}   {:<3} {:<4} {:<4} {}".format(row['num'],
                                                                        row['cid1'], row['cid1site'], row['cid1status'],
                                                                        self._dformat(row['cid1mod']),
                                                                        row['cid2'], row['cid2site'], row['cid2status'],
                                                                        self._dformat(row['cid2mod'])))

    @staticmethod
    def _isblacklist(row):
        """If a combination is blacklisted, return explanation else Fasle"""
        blacklist = {'UNK,ABA': "Unknown residue",
