...
class ChemRefSearchBase(MyConnectionBase):
    ...
    def __runRdbmsQuery(self, searchType, searchTarget, compareType='LIKE'):
        ...
        tList, cList, dList, sList, wdList = self._getDisplayAsList(displayType)
            ...
            for i1, t1 in enumerate(tList):
                for i2, t2 in enumerate(tList):
                    if i2 < i1:
                        ...
                        for jPr in [('family_prd_id', 'family_prd_id'),
                                    ('prd_id', 'prd_id'),
        ...
        if len(ljtList) > 0:
            ...
            #
            queryString = "SELECT DISTINCT " + ','.join(sList) + " FROM " + ','.join(ttList) + ' LEFT JOIN ' + \
                ','.join(ljtList) + joinConditionString + ' WHERE  ' + queryConditionString
        else:
            if (len(conditionList) > 0):
                joinConditionString = ' AND ' + ' AND '.join(conditionList)
            #
    ...
    def __standardizedSearchTarget(self, searchTargetInp, searchType):
        ...
        logger.debug("Standarizing search target '{0}' for search type '{1}'".format(searchTargetInp, searchType))
        if searchType.startswith("CCDIDX_SMILES"):
            try:
                logger.info("Input SMILES '{0}".format(searchTargetInp))
    ...
    def doSearch(self):
        ...
        #
        if (self.__verbose):
            logger.debug("searchTargetInp             = %s\n" % searchTarget)
            logger.debug("searchTypeInp               = %s\n" % searchType)
            logger.debug("queryTypeInp                = %s\n" % queryType)
            logger.debug("inputTypeInp                = %s\n" % inputType)
            logger.debug("compareTypeInp              = %s\n" % compareType)
            logger.debug("searchNameInp               = %s\n" % searchName)
        #
    ...
    def doSearchAutoComplete(self):
        ...
        if self._getQueryAutoComplete(queryType) and self._getQueryServiceType(queryType) == "rdbms":
            ...
            for sType, cType in zip(searchTypeList, compareTypeList):
                ...
                self.setResource(resourceName=resourceId)
                if self.__verbose:
                    logger.debug("Query target                   = %s\n" % searchTarget)
                    logger.debug("Query type                     = %s\n" % sType)
                    logger.debug("Query resource id              = %s\n" % resourceId)
