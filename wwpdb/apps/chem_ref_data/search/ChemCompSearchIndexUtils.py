...
class ChemCompSearchIndexUtils(object):
    ...
    def searchFormulaBounded(self, elementCounts={'C': 20, 'Cl': 4}, upperOffset=2, lowerOffset=2, excludeH=False):
        ...
        try:
            ...
            for ccId, d in self.__ccIndx.items():
                ...
                #
                mOk = True
                for type, cnt in elementCounts.items():
                    typeU = type.upper()
    ...
    def searchFormulaSubset(self, elementCounts={'C': 20, 'Cl': 4}, excludeH=False):
    ...
    def searchFormulaExact(self, elementCounts={'C': 20, 'Cl': 4}, excludeH=False):
        """ The input formula is represented as a dictionary of element type and count.
    ...
    def searchEditDistance(self, target, key, DIST_TYPE="JARO_WINKLER"):
        ...
        """
        if DIST_TYPE == "JARO":
            return self.__searchEditDistanceJaro(target, key, cutOff=.75)
        elif DIST_TYPE == "JARO_WINKLER":
            return self.__searchEditDistanceJaroWinkler(target, key, cutOff=.75)
        elif DIST_TYPE == "LEV":
            return self.__searchEditDistanceDmLev(target, key, cutOff=.5)
        else:
            return []

    def __searchEditDistanceJaro(self, target, key, cutOff=.7):
        """
        ...
        try:
            ...
            for ccId, d in self.__ccIndx.items():
                ...
                if isinstance(refV, (list, tuple, set)):
                    for tt in refV:
                        ed = jellyfish.jaro_distance(target.decode('unicode-escape'), tt.decode('unicode-escape'))
                        if ed > cutOff:
                            idList.append((ccId, ed))
                            break
                else:
                    ed = jellyfish.jaro_distance(target.decode('unicode-escape'), refV.decode('unicode-escape'))
                    if ed > cutOff:
                        idList.append((ccId, ed))
    ...
    def __searchEditDistanceJaroWinkler(self, target, key, cutOff=.7):
        """
        ...
        try:
            ...
            for ccId, d in self.__ccIndx.items():
                ...
                if isinstance(refV, (list, tuple, set)):
                    for tt in refV:
                        ed = jellyfish.jaro_winkler(target.decode('unicode-escape'), tt.decode('unicode-escape'))
                        if ed > cutOff:
                            idList.append((ccId, ed))
                            break
                else:
                    ed = jellyfish.jaro_winkler(target.decode('unicode-escape'), refV.decode('unicode-escape'))
                    if ed > cutOff:
                        idList.append((ccId, ed))
    ...
    def __searchEditDistanceDmLev(self, target, key, cutOff=.7):
        """
        """
        cidList = []
        try:
